...
| # | 主題 | 你會做到什麼 | 時間 |
|---|------|------------|------|
| [01](./01-claude-md/guide.md) | **CLAUDE.md** | 寫一份讓 Claude 自動認識你專案的設定檔，之後不用每次重新介紹 | ~15 分鐘 |
| [02](./02-skills/guide.md) | **Skills** | 建立你自己的 `/指令`，一行搞定重複的工作流程（例如 `/pr`、`/check`） | ~20 分鐘 |
| [03](./03-memory/guide.md) | **Memory** | 讓 Claude 跨對話記住你的偏好，關掉重開還記得 | ~15 分鐘 |
| [04](./04-hooks/guide.md) | **Hooks** | 設定 Claude 修改檔案後自動觸發 lint / 編譯，不用手動執行 | ~25 分鐘 |
| [05](./05-plan-mode/guide.md) | **Plan Mode + Sub-agents** | 複雜任務先讓 Claude 給計畫再動手，多個搜尋任務平行執行 | ~30 分鐘 |
| [06](./06-mcp/guide.md) | **MCP 工具** | 讓 Claude 直接查 GitHub issues、資料庫、網頁，不用你中間複製貼上 | ~30 分鐘 |
| [07](./07-schedule/guide.md) | **Schedule** | 設定每天自動產生工作摘要、每週掃描 TODO 清單，不用手動觸發 | ~20 分鐘 |

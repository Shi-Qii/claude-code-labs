...
- Chain-of-thought 與 Adaptive Thinking：讓模型自動決定推理深度、何時值得用
...
# ===== 4. Adaptive Thinking（claude-sonnet-4-6 以上）=====

def test_adaptive_thinking():
    print("\n=== Adaptive Thinking 測試 ===\n")
...
if __name__ == "__main__":
    ...
    # test_adaptive_thinking()  # 需要 claude-sonnet-4-6，取消註解後執行
```
...
**Q4：Adaptive Thinking 和舊的 Extended Thinking 有什麼差？**  
新模型（Sonnet 4.6 以後）改用 `thinking: {type: "adaptive"}`，讓模型自動判斷這個問題值不值得深度思考，不需要手動設 `budget_tokens`。舊的 `type: "enabled"` + `budget_tokens` 在新模型上已棄用。
